    ...
    @Test void packagedAgentAttachesAndInspectsARealChildJvm() throws Exception {
        ...
        try {
            ...
            try {
                ...
                String fieldSites = session.requestText(Operation.FIELD_WRITE_SITES, classId + "\nmarker");
                assertTrue(fieldSites.contains("W\t"));
                assertTrue(fieldSites.contains(Base64.getEncoder().encodeToString("setMarker".getBytes("UTF-8"))));
                String fieldTrace = session.requestText(Operation.FIELD_TRACE_START,
                        classId + "\nmarker\nmaxEvents=20;rateLimit=20;stopAfterSeconds=60;maxClasses=100");
                assertTrue(fieldTrace.startsWith("field-write-"));
                String fieldProbe = "public class FieldProbe { public static void execute(java.io.PrintStream out) { dev.whitedev.jpi.integration.AttachTarget.setMarker(\"changed\"); out.print(dev.whitedev.jpi.integration.AttachTarget.marker); } }";
                ...
                String resetField = "public class FieldReset { public static void execute(java.io.PrintStream out) { dev.whitedev.jpi.integration.AttachTarget.setMarker(\"jpi-smoke-target\"); } }";
                session.requestText(Operation.EXECUTE, resetField);
