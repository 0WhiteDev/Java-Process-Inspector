...
final class HeapObjectInspector {
    ...
    HeapObjectInspector(Instrumentation instrumentation) {
        this.instrumentation = instrumentation;
    }

    synchronized String scan(String payload) throws Exception {
    ...
    synchronized String object(String idText) throws Exception {
        long id;
        try {
        ...
        output.append('H').append('\t').append(id).append('\t').append(encoded(value.getClass().getName())).append('\t')
                .append(encoded(ClassRegistry.loaderLabel(value.getClass().getClassLoader()))).append('\t')
    ...
    private void appendInstance(StringBuilder output, Node node, long size, HeapScanConfig config) {
        output.append('O').append('\t').append(node.id).append('\t').append(encoded(node.value.getClass().getName()))
                .append('\t').append(encoded(ClassRegistry.loaderLabel(node.value.getClass().getClassLoader())))
    ...
    private static String classKey(Class<?> type) {
        return type.getName() + "@" + ClassRegistry.loaderLabel(type.getClassLoader());
    }
    ...
    private static final class ClassCount {
        ...
        ClassCount(Class<?> type) {
            ...
            loader = ClassRegistry.loaderLabel(type.getClassLoader());
        }
    }
}
